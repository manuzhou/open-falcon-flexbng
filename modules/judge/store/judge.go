...
func SendFlexbngEvent(item *model.JudgeItem) {
	if nil == item{
		return
	}
	...
	strategy.Func = "all(#1)"
	strategy.Operator = "!="
	strategy.RightValue = 0
	strategy.MaxStep = 1
	strategy.Priority = 0
	strategy.Note = item.Tags["description"]
	strategy.Metric = item.Metric
	strategy.Tpl = &model.Template{}
	strategy.Tpl.Id = 1


	event := &model.Event{
		Id:         fmt.Sprintf("s_%d_%s", strategy.Id, item.PrimaryKey()),
		Strategy:   &strategy,
